...
function handleAccountInfoUpdate(json) {
    ...
    // In Moodle 2.4 and above, tables are rendered with <thead> tags
    if ($accountsTable.children('thead').length > 0) {
        $accountRow = $accountsTable.children('tbody').find('tr:nth-child(' + tableRow + ')');
    } else {
        $accountRow = $accountsTable.find('tr:nth-child(' + tableRow + ')');
    }
...
$(document).ready(function() {
    ...
    // In Moodle 2.4 and above, tables are rendered with <thead> tags
    // Also, in some Moodle installs, tables are padded with blank rows, so check visibility
    if ($accountsTable.children('thead').length > 0) {
        numberOfAccounts = $accountsTable.find('tbody tr').filter(':visible').length;
    } else {
        numberOfAccounts = $accountsTable.find('tr:not(:first-child)').filter(':visible').length;
    }
    ...
    $userSearcher
        ...
        // jQuery UI Dialog has a higher default z-index setting than
        // the autocompleter's results list. Let's override that.
        $userSearcher.autocomplete("widget").css('z-index',1000);
